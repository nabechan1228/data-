...
const PlayerCard = ({ player }) => {
  ...
  const fetchSeasonStats = () => {
    ...
    const nameEncoded = encodeURIComponent(player.name.replace(/\s/g, ''));
    axios.get(`${API_URL}/api/season-stats/player/${player.name.trim()}`)
      .then(res => {
  ...
  const radarData = isPitcher ? [
    ...
    { subject: 'コントロール', reality: 60, vision: player.potential_score * 0.85 },
    { subject: 'スタミナ', reality: 50, vision: 65 },
    { subject: '変化球', reality: 70, vision: 75 },
    { subject: '安定感', reality: player.current_performance, vision: 80 }
  ] : [
    { subject: 'パワー', reality: player.home_runs ? Math.min(100, player.home_runs * 5) : 30, vision: player.potential_score * 0.9 },
    { subject: 'ミート力', reality: player.batting_avg ? Math.min(100, player.batting_avg * 300) : 40, vision: player.potential_score * 0.85 },
          ...
          <div className="radar-container">
            <ResponsiveContainer width="100%" height="100%">
              <RadarChart cx="50%" cy="50%" outerRadius="70%" data={radarData}>
                ...
                />
                <Radar name="Vision (Potential)" dataKey="vision" stroke="#F59E0B" strokeWidth={3} fill="#F59E0B" fillOpacity={0.1} />
                <Radar name="Reality (Current)" dataKey="reality" stroke="#E50012" strokeWidth={2} fill="#E50012" fillOpacity={0.6} />
              </RadarChart>
            </ResponsiveContainer>
          </div>
